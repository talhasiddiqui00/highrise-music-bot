...
class Bot(BaseBot):
    ...
    async def process_tip_queue_worker(self):
        while True:
            ...
            try:
                ...
                if reason == "welcome":
                    await self.highrise.chat(f"ð @{username}, enjoy your 1g welcome bonus!")
                elif reason == "stay_reward":
                    await self.highrise.chat(f"â° @{username} received 1g for supporting the room with their stay-time! ð")
    ...
    async def start_announcement_loop(self) -> None:
        announcements = [
            "â¨ <color=#FF0000><b>Welcome!</b></color> Tip <color=#FFD700><b>500g+</b></color> to the Bot for permanent VIP access! ðð",
            "ð¡ <color=#00FF00><b>Need help?</b></color> Type <color=#00FFFF><b>!help</b></color> for commands and <color=#FF00FF><b>!list</b></color> for emotes! ð­",
            "ð¶ <color=#0000FF><b>Having fun?</b></color> Just type an emote name (like <color=#FFA500><b>cozy nap</b></color>) to start looping! ðºð"
        ]
        while True:
            try:
    ...
    async def handle_welcome_flow(self, user: User):
        ...
        try:
            await self.highrise.chat(f"ð Welcome to the room @{user.username}! Type '!help' for information.")
        except Exception:
            pass
    ...
    async def on_tip(self, sender: User, receiver: User, tip: CurrencyItem | Item) -> None:
        ...
        if isinstance(tip, CurrencyItem) and receiver.id == self.bot_id:
            ...
            if tip.amount >= 500:
                ...
                await self.highrise.chat(f"ð VIP PROMOTION: @{sender.username} has unlocked Lifetime VIP permissions! ð")
            else:
    ...
    async def command_handler(self, user: User, message: str, source: str):
        ...
        if normalized_msg in EMOTE_MAP:
            ...
            await self.respond(user, f"ð­ Now looping! Type '!stop' to end.", source)
            return

        # 4. Standard Commands
        if clean_msg == "!help":
            help_text = "â¡ Commands: !list | !stop | !vip | !down"
            if is_owner:
            ...
            await self.highrise.send_whisper(user.id, "ð¡ Just type any of these names in chat to loop the emote! Type '!stop' to cancel.")
        ...
        if clean_msg.startswith("!giveall "):
                ...
                if amount_str in TIP_MAP:
                    ...
                    for u, _ in room_users.content:
                        if u.id != self.bot_id:
                            ...
                            count += 1
                    await self.respond(user, f"ð¸ Queued {amount_str} tip to {count} users in the room!", source)
                else:
                    await self.respond(user, "â Invalid amount. Use 1g, 5g, 10g, 50g, 100g, 500g, 1k, 5k, or 10k.", source)
            except IndexError:
                await self.respond(user, "â Invalid format. Use: !giveall 10g", source)
            return
                ...
                if amount_str in TIP_MAP:
                    ...
                    for u, _ in room_users.content:
                        if u.username.lower() == target_name:
                            await self.tip_queue.put((u.id, TIP_MAP[amount_str], u.username, "manual_tip"))
                            await self.respond(user, f"ð¸ Queued {amount_str} tip to @{u.username}", source)
                            return
                    await self.respond(user, "â User not found in the room.", source)
                else:
                    await self.respond(user, "â Invalid amount. Use 1g, 5g, 10g, 50g, 100g, 500g, 1k, 5k, or 10k.", source)
            else:
                await self.respond(user, "â Invalid format. Use: !give @username 10g", source)
            return
                ...
                for u, _ in room_users.content:
                    if u.username.lower() == target_name:
                        if u.id not in self.vip_users:
                            ...
                            self.save_database_file()
                            await self.respond(user, f"â @{u.username} has been manually granted VIP status!", source)
                        else:
                            await self.respond(user, f"â ï¸ @{u.username} is already a VIP.", source)
                        return
                await self.respond(user, "â User not found in the room.", source)
            except IndexError:
                await self.respond(user, "â Invalid format. Use: !givevip @username", source)
            return
                ...
                for u, _ in room_users.content:
                    if u.username.lower() == target_name:
                        if u.id in self.vip_users:
                            ...
                            self.save_database_file()
                            await self.respond(user, f"ð« @{u.username} has had their VIP status revoked.", source)
                        else:
                            await self.respond(user, f"â ï¸ @{u.username} is not a VIP.", source)
                        return
                await self.respond(user, "â User not found in the room.", source)
            except IndexError:
                await self.respond(user, "â Invalid format. Use: !removevip @username", source)
            return
            ...
            await self.highrise.send_whisper(user.id, f"ð Total VIPs ({len(vip_names)}): {vip_string}")
            return
            ...
            try:
                ...
                if isinstance(position, Position):
                    ...
                    await self.respond(user, "ð Bot position updated successfully!", source)
            except Exception as e:
            ...
            try:
                ...
                await self.highrise.send_whisper(user.id, f"ð° Balance: {gold}g")
            except Exception:
                pass
            return
